...
/**
 * Based on the BLS contracts from the elliptic-curve-solidity project.
 * Find the original source here: https://github.com/witnet/elliptic-curve-solidity/blob/master/contracts/EllipticCurve.sol
 */

/**
 * @title Library to help aggregate signatures based on eliptic curves
 *     @dev There are many other helpful functions for eliptic curves that have been excluded to focus only on what is needed for aggregation
 */
library EllipticCurve {
    /**
     * sum all the points in the array
     * @param points an array of uint256[2], representing an (x,y) of a point
     * @param _pp the modulus of the curve
     * @return ret the sum of all points
     */
    function sum(uint256[2][] memory points, uint256 _pp) internal pure returns (uint256[2] memory ret) {
        uint256 x = points[0][0];
        uint256 y = points[0][1];
        uint256 z = 1;

        for (uint256 i = 1; i < points.length; i++) {
            (x, y, z) = jacAdd(x, y, z, points[i][0], points[i][1], 1, _pp);
        }
