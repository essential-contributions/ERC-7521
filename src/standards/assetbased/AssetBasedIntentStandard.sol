...
import {AssetBasedIntentSegment} from "./AssetBasedIntentSegment.sol";
...
contract AssetBasedIntentStandard is EntryPointTruster, AssetBasedIntentDelegate, IIntentStandard {
    ...
    function validateIntentSegment(bytes calldata segmentData) external pure {
        AssetBasedIntentSegment calldata segment = parseIntentSegment(segmentData);
    ...
    function executeIntentSegment(
        ...
        AssetBasedIntentSegment calldata segment = parseIntentSegment(intent.intentData[segmentIndex]);

        //check asset requirements
        _checkAssetRequirements(segment, segmentIndex, evaluateAt, startingBalances, intent.sender);
        ...
        if (segmentIndex + 1 < intent.intentData.length && intent.intentData[segmentIndex + 1].length > 0) {
            AssetBasedIntentSegment calldata nextSegmentData = parseIntentSegment(intent.intentData[segmentIndex + 1]);
            startingBalances = _recordStartingBalances(nextSegmentData, intent.sender);
        }

        //execute calldata
        if (segment.callData.length > 0) {
            Exec.callAndRevert(intent.sender, segment.callData, REVERT_REASON_MAX_LEN);
        }
