...
/**
 * Eth Release Intent Curve struct
 ...
 */
struct EthReleaseIntentCurve {
    uint96 flags;
    int256[] params;
}

/**
 * Validate EthReleaseIntentCurve params and flags.
 */
function validate(EthReleaseIntentCurve calldata curve) pure {
 ...
 */
function evaluate(EthReleaseIntentCurve calldata curve, uint256 x) pure returns (int256 val) {
    int256 sx = int256(x);
    if (parseCurveType(curve) == CurveType.CONSTANT) {
        val = curve.params[0];
    } else if (parseCurveType(curve) == CurveType.LINEAR) {
 ...
 */
function parseCurveType(EthReleaseIntentCurve calldata curve) pure returns (CurveType) {
 ...
 */
function parseEvaluationType(EthReleaseIntentCurve calldata curve) pure returns (EvaluationType) {
 ...
 */
function isRelativeEvaluation(EthReleaseIntentCurve calldata curve) pure returns (bool) {
