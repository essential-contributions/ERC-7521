...
contract EntryPoint is IEntryPoint, NonceManager, ReentrancyGuard, EmbeddedIntentStandards {
    ...
    mapping(bytes32 => DeployableIntentStandard) private _registeredStandards;
    ...
    function _executeIntent(
        ...
        if (intent.sender != address(0) && intent.intentData.length > 0) {
            ...
            if (isEmbeddedIntentStandard(standardId)) {
                ...
                if (intentStandard == DeployableIntentStandard(address(0))) {
                ...
                bool success = Exec.call(
                    ...
                    abi.encodeWithSelector(
                        DeployableIntentStandard.executeIntentSegment.selector,
                        solution,
                        executionIndex,
                        segmentIndex,
                        contextData
    ...
    function validateIntent(UserIntent calldata intent) external view {
        ...
        for (uint256 i = 0; i < intent.intentData.length; i++) {
            ...
            if (isEmbeddedIntentStandard(standardId)) {
                ...
                DeployableIntentStandard standard = _registeredStandards[standardId];
                if (standard == DeployableIntentStandard(address(0))) {
