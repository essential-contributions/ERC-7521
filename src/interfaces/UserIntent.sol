...
/**
 * User Intent struct
 * @param type the intent type (format).
 * @param sender the sender account of this request.
 * @param nonce unique value the sender uses to verify it is not a replay.
 * @param timestamp the time when the intent was created.
 * @param intentData the intent data specific to the intents type.
...
library UserIntentLib {
    ...
    function _pack(UserIntent calldata intent) private pure returns (bytes memory ret) {
        bytes32 intentType = intent.intentType;
        ...
        return abi.encode(intentType, sender, nonce, timestamp, intentDataHash);
    }
}
